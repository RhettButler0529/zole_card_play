...
const closeResultNotification2 = (req, res) => {
  ...
  cors(req, res, () => {
    ...
    admin.auth().verifyIdToken(tokenId)
      ...
      //  log(roomId, `closeResultNotification: decoded.uid: ${decoded.uid}`);
        ...
        const promise1 = admin.database(roomsDb).ref(`rooms/${roomId}/playersList/playerList`).once('value');
        const promise2 = admin.database(roomsDb).ref(`gameSettings`).once('value');
        const promise3 = admin.database(roomsDb).ref(`rooms/${roomId}/nextTimestamp`).once('value');
        ...
        Promise.all([promise1, promise2, promise3]).then((promiseRes) => {
          ...
          if (nextTimestamp < Date.now()) {
            admin.database(roomsDb).ref(`rooms/${roomId}/globalParams`).transaction((globalParams) => {
            ...
            //  log(roomId, `closeResultNotification: gameState: ${globalParams.gameState}`);
              ...
              if (result.committed) {
                ...
                admin.database().ref(`rooms/${roomId}/globalParams`).set(committedGlobalParams);
                ...
                if (lowBalPlayers) {
                  ...
                  admin.database(roomsDb).ref(`rooms/${roomId}/nextTimestamp`).set(Date.now() + 1000 * 15);
...
module.exports = closeResultNotification2;
