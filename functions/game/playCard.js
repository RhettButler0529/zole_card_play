    ...
    admin.auth().verifyIdToken(tokenId)
      .then((decoded) => {
        ...
        admin.database().ref(`rooms/${roomId}/playersList/playerList/${decoded.uid.toString()}`).once('value', (snapshot) => {
          ...
          const promise1 = admin.database().ref(`rooms/${roomId}/globalParams/fastGame`).once('value');
          const promise2 = admin.database().ref(`rooms/${roomId}/globalParams/party`).once('value');
          const promise3 = admin.database().ref(`rooms/${roomId}/globalParams/gameState`).once('value');
          const promise4 = admin.database().ref(`rooms/${roomId}/curRnd/firstToGo`).once('value');
          const promise5 = admin.database().ref(`rooms/${roomId}/curRnd/currentTurn`).once('value');
          //  const promise6 = admin.database().ref(`rooms/${roomId}/curRnd/currentTable`).once('value');
          const promise7 = admin.database().ref('gameSettings').once('value');
          const promise8 = admin.database().ref(`rooms/${roomId}/players/${playerPosition}/cards`).once('value');

          Promise.all([promise1, promise2, promise3, promise4, promise5, promise7, promise8]).then((results) => {
            let fastGame;
            let party;
            let gameState;
            let firstToGo;
            ...
            if (playerPosition) {
              ...
              if (gameState === 'burry') {
                if (currentTurn === playerPosition && playedCard) {
                      ...
                      admin.database().ref(`rooms/${roomId}/beatCardPoints/${playerPosition}`)
                        .transaction(pnts => (pnts || 0) + pointsChange).then((result2) => {
                          if (!result2.committed) {
                            log(roomId, `playCard: Error updating beatCardPoints, error: ${result2.error}`);
                  ...
                  //  const promise1 = admin.database().ref(`rooms/${roomId}/nextTimestamp`).set((Date.now() + 1000 * speed) + 500);
                  //  const promise2 = admin.database().ref(`rooms/${roomId}/globalParams/gameState`).update('play');
                  //  const promise3 = admin.database().ref(`rooms/${roomId}/curRnd`).update({ currentTurn: nextPosition });
                  //  const promise4 = admin.database().ref(`rooms/${roomId}/globalParams`).update({ currentHand: 1 });
                  ...
                  //    console.log(promisesRes);
                      ...
                      admin.database().ref(`rooms/${roomId}`).update(updates).then(() => {
                        ...
                        admin.database().ref(`rooms/${roomId}/beatCardPoints/${playerPosition}`)
                          .transaction(pnts => (pnts || 0) + pointsChange).then((result2) => {
                            if (!result2.committed) {
                              log(roomId, `playCard: Error updating beatCardPoints #2, error: ${result2.error}`);
