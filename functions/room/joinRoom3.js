const joinRoom3 = (req, res) => {
  const cors = require('cors')({ origin: true });
  const { admin, adminLogsDb, roomsPublicDb, userStatsDB, roomsDb } = require('../admin');
    ...
    roomState.isIdle(roomId).then(() => {
      ...
      admin.auth().verifyIdToken(tokenId)
        .then((decoded) => {
          ...
          const promise1 = admin.database(roomsDb).ref(`rooms/${roomId}/players`).once('value');
          const promise2 = admin.database(roomsDb).ref(`rooms/${roomId}/globalParams/roomClosed`).once('value');
          ...
          Promise.all([promise1, promise2, promise3]).then((results) => {
            ...
            Promise.all(promises).then((promiseRes) => {
              ...
              const promise5 = admin.database(roomsDb).ref(`rooms/${roomId}/globalParams/bet`).once('value');
              const promise6 = admin.database(roomsDb).ref(`rooms/${roomId}/globalParams/privateRoom`).once('value');
              const promise7 = admin.database(roomsDb).ref(`rooms/${roomId}/globalParams/password`).once('value');
              const promise8 = admin.database(roomsDb).ref(`rooms/${roomId}/globalParams/proGame`).once('value');
              ...
              Promise.all([promise4, promise5, promise6, promise7, promise8]).then((promiseRes2) => {
                ...
                if (userData.bal >= balNeeded) {
                  ...
                  admin.database(roomsDb).ref(`rooms/${roomId}/players`).transaction((players2) => {
                    if (!players2) {
                      return players2;
                    }
                    ...
                    //  admin.database().ref(`rooms/${roomId}/players`).set(result.snapshot.val());

                      const lastPosition = Object.keys(result.snapshot.val()).pop()

                      admin.database(roomsDb).ref(`rooms/${roomId}/players`).once('value', (playersSnapshot) => {
                        ...
                        //  admin.database().ref(`rooms/${roomId}/globalParams/talking`).set(players3.player1.uid);
                          admin.database(roomsDb).ref(`rooms/${roomId}/globalParams/talking`).set(players3.player1.uid);
                        }
                      ...
                      //  if (players3 && players3[lastPosition] && players3[lastPosition].uid && players3[lastPosition].uid === decoded.uid) {
                        /*  admin.database().ref(`rooms/${roomId}/playersList/${lastPosition}`).update({
                          ...
                          admin.database(roomsDb).ref(`rooms/${roomId}/playersList/${lastPosition}`).update({
                            uid: decoded.uid,
                            name,
                            shortName,
                            photo: userData.photo,
                        ...
                        //  });
                          ...
                          admin.database(roomsDb).ref(`rooms/${roomId}/playersList/playerList`).update({
                            [decoded.uid]: lastPosition,
                          });
                          ...
                          if (players3.player1 && players3.player2 && players3.player3) {
                            ...
                            Object.keys(players3).map((key) => {
                              ...
                              //  });
                                ...
                                admin.database(roomsDb).ref(`rooms/${roomId}/playersList/${key}`).update({
                                  bal: newBal,
                                });

                                admin.database().ref(`users/${players3[key].uid}/activeRoom`).set(roomId);
                                ...
                                admin.database(userStatsDB).ref(`userBalHistory/${players3[key].uid}`).push({
                                  time: Date.now(),
                                  type: 'joinPrice',
                                  roomId,
                                  change: `-${joinPrice}`,
                ...
                //  return res.status(200).send({ data: { status: 'error', error: 'insufficient balance', balNeeded } });
                 ...
                 if (joinedRoomId === roomId) {
                   ...
                   const promise4 = admin.database(roomsDb).ref(`rooms/${joinedRoomId}/nextTimeStamp`).once('value');
                   const promise5 = admin.database(roomsDb).ref(`rooms/${joinedRoomId}/globalParams/roomClosed`).once('value');
